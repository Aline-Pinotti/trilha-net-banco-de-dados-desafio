...
SELECT f.Nome as Filme, g.Genero
  FROM Filmes f
  LEFT JOIN FilmesGenero fg ON f.Id = fg.IdFilme -- dessa forma também traz os filmes sem gênero
...
SELECT f.Nome as Filme, a.PrimeiroNome +' '+ a.UltimoNome as Ator, e.Papel
  FROM Filmes f
  RIGHT JOIN ElencoFilme e ON f.Id = e.IdFilme -- RIGHT JOIN, para não trazer filmes que "não tenham" atores
